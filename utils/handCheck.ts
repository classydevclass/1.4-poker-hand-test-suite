...
export const hasStraight = (cards: Card[]): HandCheckResults<{ highCard: Card, numInARow: number }> => {
...
const addHighestRemainingCardsToHand = (cardsUsedForHand: Card[], allCards: Card[]) => {
...
export const determinePlayerHand = (cards: Card[]): PlayerHandResults => {
    ...
    if (playerStraightFlush.result) {
        ...
        playerHandCards = [...cards.filter(card => card.value === bestTripleValue), ...cards.filter(card => card.value === bestPairValue)];
